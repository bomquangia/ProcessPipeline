...
RunPipeline <- function(study_id, arg, add_meta = FALSE) {
  ...
  rBCS::ExportSeurat(obj, output_path, author = "support@bioturing.com", clustering.name = "bioturing_graph", overwrite=FALSE, unique.limit=200)
  
  log4r::info(file_logger, paste("Renaming original metadata"))
  RenameOriginalMetadata(arg$output_dir, output_path, study_id)
